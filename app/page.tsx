 ...
 * - NEW: Full-screen onboarding wizard (Welcome → Connect → Split Paycheck → Lock Rules → Keyholder → Review)
...
const [unlockModal, setUnlockModal] = useState<null | { vaultId: string }>(null);
  ...
  const [netPay, setNetPay] = useState(2200); // Example paycheck
  ...
  const finishOnboarding = () => {
    // Apply allocations to vaults (mock): create/update vaults by percentages of net pay
    const rentAdd = Math.round((alloc.rent / 100) * netPay);
    const emerAdd = Math.round((alloc.emergency / 100) * netPay);

    setVaults((prev) => prev.map(v => v.id === "rent" ? { ...v, saved: Math.min(v.target, v.saved + rentAdd), locked: lockUntilDue ? Math.min(v.target, v.locked + rentAdd) : v.locked } : v)
...
// Move money from UNLOCKED -> LOCKED for a vault
function lockFunds(vaultId: string, amount: number) {
  setVaults(prev =>
    prev.map(v => {
      if (v.id !== vaultId) return v;
      const lockable = Math.max(0, v.saved - v.locked); // only from unlocked
...
// Withdraw from UNLOCKED portion to bank
...
// Move from one vault's UNLOCKED portion to another vault's SAVED
...
// Lock an amount (immediate). Locks up to available (saved - locked)
function lockFunds(vaultId: string, amount: number) {
  setVaults(prev =>
    prev.map(v => {
      if (v.id !== vaultId) return v;
...
// Withdraw to bank from the **unlocked** portion only
function withdrawToBank(vaultId: string, amount: number) {
  setVaults(prev =>
    prev.map(v => {
      if (v.id !== vaultId) return v;
...
// --- Simulated Keyholder Notification (SMS / Link) ---
function sendKeyholderNotification(req: {
  id: string;
  vaultId: string;
  amount: number;
...
Darian requested an early unlock of $${req.amount} from their ${req.vaultId} vault.
Reason: "${req.reason || "No reason provided."}"
...
Download the LockBox app:
https://lockbox.app/download
  `.trim();

  console.log("📱 Simulated Keyholder SMS:", message);
  alert("Simulated SMS sent to Keyholder.\nCheck console for message details.");
}
  ...
  if (!keyholder.enabled) {
    ...
    setVaults((prev) =>
      prev.map((v) =>
        v.id === vaultId ? { ...v, locked: Math.max(0, v.locked - amount) } : v
      )
    );
    setShowGoal(true);
  }
              ...
              <section className="space-y-3">
                ...
                <Card className="p-4">
                  <div className="flex items-center gap-4">
                    ...
                    <button onClick={() => setTab("banker")} className="px-3 py-1.5 rounded-full text-sm bg-gray-900 text-white">Open</button>
                  </div>
                </Card>
                <Card className="p-4">
                  <div className="flex items-center gap-4">
                    ...
                    <button onClick={startOnboarding} className="px-3 py-1.5 rounded-full text-sm bg-emerald-600 text-white">Start</button>
                  </div>
                </Card>
              </section>
            </motion.div>
                  ...
                  <Card key={v.id} className="p-4">
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <div className="text-sm text-gray-500">{currency(v.target)}{v.dueDays ? ` due in ${v.dueDays} days` : " target"}</div>
                      </div>
                      <div className="text-right">
                        <div className="text-sm text-gray-500">{currency(v.locked)} locked</div>
                      </div>
                    </div>
                    <div className="mt-3"><Progress value={pct} /></div>
                    <div className="mt-3 flex items-center justify-between">
                      <div className="text-sm text-gray-600">Saved: <span className="font-medium text-gray-900">{currency(v.saved)}</span></div>
...
>
  ...
  {/* NEW: Lock funds */}
  <button
    onClick={() => setLockModal({ vaultId: v.id })}
    className="px-3 py-1.5 rounded-full text-sm flex items-center gap-1 bg-white border"
    title="Lock some of your unlocked balance"
  >
  ...
  {v.locked > 0 && (
    <button
      onClick={() => setUnlockModal({ vaultId: v.id })}
      className="px-3 py-1.5 rounded-full text-sm flex items-center gap-1 bg-white border"
      title="Ask to unlock some of your locked balance"
    >
      <Unlock className="h-4 w-4" /> Request unlock
              ...
              <Card className="p-4 border-dashed">
                <button onClick={() => {
                    ...
                    setVaults((v) => [...v, { id, name: "New goal", target: 500, locked: 0, saved: 0, dueDays: 30 }]);
...
<Card className="p-4">
      ...
      {requests.map((r) => {
        ...
        return (
          ...
          >
            ...
            <div className={`px-2 py-1 rounded-md text-xs ${statusStyles}`}>
              {r.status}
            </div>
          </div>
        );
  ...
  {/* optional demo-only inline controls to flip pending to approved/rejected */}
          ...
          {tab === "rewards" && (
            <motion.div key="rewards" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="px-4 py-5 space-y-4">
              ...
              <Card className="p-5 bg-[#0E3559] text-white">
                <div className="flex items-center gap-4">
                  <div className="h-12 w-12 rounded-2xl bg-white/10 grid place-items-center"><Star className="h-6 w-6"/></div>
                  <div>
              ...
              <Card className="p-4">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {["On-time rent", "Impulse slayer", "Streak x7"].map((b) => (
                    <div key={b} className="aspect-square rounded-xl border grid place-items-center text-center text-sm p-2">{b}</div>
                  ))}
                </div>
              </Card>
            </motion.div>
...
<UnlockModal
  ...
  keyholderEnabled={keyholder.enabled}
...
<AddFundsModal
  ...
  onAdd={(amount) => {
    const id = addFundsModal?.vaultId;
    if (!id) return;
    setVaults((prev) =>
      prev.map((v) =>
        v.id === id
          ? { ...v, saved: Math.min(v.target, v.saved + amount) }
          : v
      )
    );
  }}
      ...
      {open && (
        <motion.div className="fixed inset-0 z-40 grid place-items-center bg-black/40 p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <motion.div initial={{ scale: 0.96, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.96, opacity: 0 }} className="w-full max-w-sm rounded-3xl bg-white p-6 text-center">
            ...
            <button onClick={onClose} className="w-full py-3 rounded-xl bg-emerald-600 text-white text-sm font-medium">Continue</button>
          </motion.div>
        </motion.div>
      )}
    </AnimatePresence>
...
function TransferModal({
  ...
  // Hooks MUST always be called, regardless of conditional returns
  const [mode, setMode] = useState<"bank" | "vault">("bank");
  const [amount, setAmount] = useState(100);
  const [toVaultId, setToVaultId] = useState<string>("");

  // Compute with null-safe fallbacks so hooks run before any early return
  const unlocked = Math.max(
    0,
    (sourceVault?.saved ?? 0) - (sourceVault?.locked ?? 0)
  );

  const otherVaults = useMemo(
    () => (sourceVault ? vaults.filter((v) => v.id !== sourceVault.id) : []),
    [vaults, sourceVault?.id]
  );

  useEffect(() => {
    // Guard inside the effect instead of skipping the hook
    if (mode === "vault" && otherVaults.length > 0 && !toVaultId) {
      setToVaultId(otherVaults[0].id);
    }
  }, [mode, otherVaults, toVaultId]);

  // Only now is it safe to early-return
        ...
        >
          ...
          >
            ...
            <div className="text-sm text-gray-500 mb-4">
              Funds will stay locked until approved
              {keyholderEnabled ? " by your Keyholder." : "."}
            </div>
...
function LockModal({
  ...
  open: boolean;
  onClose: () => void;
  vault: { id: string; name: string; saved: number; locked: number } | null;
...
function AddFundsModal({
  ...
  open: boolean;
  onClose: () => void;
  vault: { id: string; name: string; saved: number; target: number } | null;
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-gray-500 mb-4">
              Enter an amount to deposit into <b>{vault.name}</b> from your connected bank account.
            </p>
            ...
            <div className="mt-5 flex gap-2">
              ...
              <button
                onClick={() => setAmount(vault.target - vault.saved)}
                className="px-3 py-1.5 rounded-full bg-gray-100 text-sm"
              >
                Max
              </button>
      ...
      {open && (
        ...
        >
          <motion.div
            ...
            className="w-full max-w-sm rounded-3xl bg-white p-6"
          >
            <div className="text-lg font-semibold mb-2">Withdraw to bank</div>
            <div className="text-sm text-gray-500 mb-3">
              Available to withdraw: <b>${unlocked}</b>
            </div>
      ...
      <div className="fixed bottom-16 left-0 right-0">
        <div className="mx-auto max-w-md px-4">
          <Card className="p-2 flex items-center gap-2">
            ...
            <button onClick={send} className="px-3 py-2 rounded-xl bg-gray-900 text-white text-sm">Send</button>
            <button onClick={onCelebrate} className="px-3 py-2 rounded-xl bg-emerald-600 text-white text-sm">Celebrate</button>
          </Card>
        </div>
      </div>
    </motion.div>
...
function OnboardingWizard(props: {
  ...
  netPay: number; setNetPay: (n: number) => void;
  alloc: { rent: number; emergency: number; spending: number };
  setAlloc: (a: { rent: number; emergency: number; spending: number }) => void;
  lockUntilDue: boolean; setLockUntilDue: (b: boolean) => void;
  keyholder: { enabled: boolean; contact: string; rule: string };
  setKeyholder: (k: { enabled: boolean; contact: string; rule: string }) => void;
  onClose: () => void;
  onFinish: () => void;
}) {
  const { open, step, setStep, connected, setConnected, netPay, setNetPay, alloc, setAlloc, lockUntilDue, setLockUntilDue, keyholder, setKeyholder, onClose, onFinish } = props;
        ...
        <motion.div className="fixed inset-0 z-50 bg-white overflow-y-auto" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <div className="mx-auto max-w-md px-4 py-4">
            ...
            {step === OnbStep.Welcome && (
              <Card className="p-5">
                ...
                <ul className="text-sm text-gray-600 space-y-2 mb-4">
                  <li className="flex items-center gap-2"><Banknote className="h-4 w-4"/> Connect income</li>
                  <li className="flex items-center gap-2"><ArrowRightLeft className="h-4 w-4"/> Set your split</li>
                  <li className="flex items-center gap-2"><Lock className="h-4 w-4"/> Lock until due date (optional)</li>
                  <li className="flex items-center gap-2"><Users className="h-4 w-4"/> Add a Keyholder (optional)</li>
                </ul>
                <div className="flex gap-3">
                  <button onClick={() => setStep(OnbStep.Connect)} className="flex-1 py-3 rounded-xl bg-emerald-600 text-white">Start</button>
                </div>
              </Card>
            )}
            ...
            {step === OnbStep.Connect && (
              <Card className="p-5">
                <div className="flex items-center gap-3 mb-2"><Banknote className="h-6 w-6 text-emerald-600"/><div className="text-lg font-semibold">Connect your income</div></div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  {[["Chase"], ["BofA"], ["Wells"], ["Cash App"], ["Citi"], ["ADP"]].map(([name], i) => (
                    <button key={i} onClick={() => setConnected(true)} className={`p-3 rounded-xl border ${connected ? "border-emerald-500" : ""}`}>{name}</button>
                  ))}
                </div>
                <div className="mb-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => setStep(OnbStep.Welcome)} className="py-3 rounded-xl border">Back</button>
                  <button disabled={!connected} onClick={() => setStep(OnbStep.Split)} className={`py-3 rounded-xl text-white ${connected ? "bg-emerald-600" : "bg-gray-300"}`}>{connected ? "Continue" : "Choose an account"}</button>
                </div>
              </Card>
            )}
            ...
            {step === OnbStep.Split && (
              <Card className="p-5">
                <div className="flex items-center gap-3 mb-2"><ArrowRightLeft className="h-6 w-6 text-emerald-600"/><div className="text-lg font-semibold">Split your paycheck</div></div>
                ...
                <div className="mt-3 text-sm flex items-center justify-between">
                  <div className="text-gray-600">Allocated: <span className={`${remaining===0?"text-emerald-700":"text-amber-600"}`}>{100-remaining}%</span></div>
                  <div className="text-gray-600">Remaining: <span className={`${remaining===0?"text-emerald-700":"text-amber-600"}`}>{remaining}%</span></div>
                </div>

                <div className="grid grid-cols-2 gap-3 mt-5">
                  <button onClick={() => setStep(OnbStep.Connect)} className="py-3 rounded-xl border">Back</button>
                  <button disabled={remaining!==0} onClick={() => setStep(OnbStep.LockRules)} className={`py-3 rounded-xl text-white ${remaining===0?"bg-emerald-600":"bg-gray-300"}`}>Continue</button>
                </div>
              </Card>
            )}
            ...
            {step === OnbStep.LockRules && (
              <Card className="p-5">
                <div className="flex items-center gap-3 mb-2"><Lock className="h-6 w-6 text-emerald-600"/><div className="text-lg font-semibold">Lock rules</div></div>
                ...
                <div className="flex items-center justify-between p-3 rounded-xl border mb-3">
                  ...
                  <label className="inline-flex items-center cursor-pointer">
                    ...
                    <div className={`w-11 h-6 rounded-full ${lockUntilDue?"bg-emerald-600":"bg-gray-300"} relative transition`}> <span className={`absolute top-0.5 ${lockUntilDue?"left-6":"left-0.5"} h-5 w-5 bg-white rounded-full transition`} /> </div>
                  </label>
                </div>
                <div className="p-3 rounded-xl border text-sm text-gray-600 flex items-center gap-2"><Clock className="h-4 w-4"/> If you need it early, request an early unlock. The Banker will review your reason.</div>
                <div className="grid grid-cols-2 gap-3 mt-5">
                  <button onClick={() => setStep(OnbStep.Split)} className="py-3 rounded-xl border">Back</button>
                  <button onClick={() => setStep(OnbStep.Keyholder)} className="py-3 rounded-xl bg-emerald-600 text-white">Continue</button>
                </div>
              </Card>
            )}
            ...
            {step === OnbStep.Keyholder && (
              <Card className="p-5">
                <div className="flex items-center gap-3 mb-2"><Users className="h-6 w-6 text-emerald-600"/><div className="text-lg font-semibold">Add a Keyholder (optional)</div></div>
                ...
                <div className="flex items-center gap-2 mb-3">
                  <label className="inline-flex items-center cursor-pointer">
                    ...
                    <div className={`w-11 h-6 rounded-full ${keyholder.enabled?"bg-emerald-600":"bg-gray-300"} relative transition`}> <span className={`absolute top-0.5 ${keyholder.enabled?"left-6":"left-0.5"} h-5 w-5 bg-white rounded-full transition`} /> </div>
                  </label>
                ...
                <div className="grid grid-cols-2 gap-3 mt-5">
                  <button onClick={() => setStep(OnbStep.LockRules)} className="py-3 rounded-xl border">Back</button>
                  <button onClick={() => setStep(OnbStep.Review)} className="py-3 rounded-xl bg-emerald-600 text-white">Continue</button>
                </div>
              </Card>
            )}
            ...
            {step === OnbStep.Review && (
              <Card className="p-5">
                <div className="flex items-center gap-3 mb-2"><CheckCircle2 className="h-6 w-6 text-emerald-600"/><div className="text-lg font-semibold">Review & finish</div></div>
                <div className="space-y-3 text-sm">
                  <div className="flex items-center justify-between p-3 rounded-xl border"><span>Account linked</span>{connected? <span className="text-emerald-700 flex items-center gap-1"><Check className="h-4 w-4"/> Connected</span> : <span className="text-amber-700 flex items-center gap-1"><CircleSlash className="h-4 w-4"/> Not connected</span>}</div>
                  <div className="p-3 rounded-xl border"><div className="font-medium mb-1">Split</div><ul className="space-y-1 text-gray-700"><li>Rent safe-deposit box — {alloc.rent}% ({currency(Math.round(netPay*alloc.rent/100))})</li><li>Emergency fund — {alloc.emergency}% ({currency(Math.round(netPay*alloc.emergency/100))})</li><li>Spending — {alloc.spending}% ({currency(Math.round(netPay*alloc.spending/100))})</li></ul></div>
                  <div className="p-3 rounded-xl border flex items-center justify-between"><span>Lock until rent due</span><span className="text-gray-700">{lockUntilDue? "On" : "Off"}</span></div>
                  <div className="p-3 rounded-xl border flex items-center justify-between"><span>Keyholder</span><span className="text-gray-700">{keyholder.enabled? (keyholder.contact || "Enabled") : "Disabled"}</span></div>
                </div>
                <div className="grid grid-cols-2 gap-3 mt-5">
                  <button onClick={() => setStep(OnbStep.Keyholder)} className="py-3 rounded-xl border">Back</button>
                  <button onClick={onFinish} className="py-3 rounded-xl bg-emerald-600 text-white">Finish setup</button>
                </div>
              </Card>
            )}
  ...
  return (
    <div className="mb-4">
      <div className="flex items-center justify-between mb-1">
        ...
        <div className="text-sm font-medium">{value}% <span className="text-gray-500">({currency(preview)})</span></div>
      </div>
