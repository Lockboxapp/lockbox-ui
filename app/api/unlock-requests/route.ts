...
// Sprint 5 — board decision: a pending unlock request lives for
// 24 hours. If the keyholder doesn't act, the cron at
// /api/cron/relock transitions PENDING→EXPIRED and stamps a 24h
// cooldown so the owner can't ping a fresh keyholder immediately.
...
export async function POST(req: NextRequest) {
  try {
    ...
    // For the session.user.* accesses further down (PostHog,
    // sendUnlockRequestToKeyholder ownerName, etc.) we still need
    // the user's display fields. Load them once.
    ...
    // Create the unlock request. Sprint 5: stamp a 24h
    // `expiresAt` so the cron can transition stale PENDING rows
    // to EXPIRED. `keyholderRelationshipId` records the owner's
    // single-keyholder choice (verified above in the validation
    // path so this is the active relationship for the box).
    ...
    const unlockRequest = await prisma.unlockRequest.create({
      data: {
        ...
        keyholderRelationshipId: keyholderRelationshipId ?? null,
      },
    });
